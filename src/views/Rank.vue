<template>
  <div class="container mx-auto mt-10">
    ...
    <div class="relative bg-white shadow-xl rounded-lg w-full">
      <div v-if="errored" class="backdrop-blur-sm w-full h-full flex items-center text-red-600 text-lg justify-center z-10 absolute">
      ...
      <div class="p-3" v-else>
        <div class="rank-list">
          <router-link v-for="(item, index) in items" :key="item.id" :to="`/${item.name}`">
            <div class="border border-gray-300 border-b-0 p-2 hover:bg-gray-50 cursor-pointer justify-between inline-flex w-full">
        ...
        <Pagination :page-size="perPage" :total-pages="totalPages" :total-items="totalItems" @changePage="onChangePage"/>
      </div>
    </div>
  </div>
</template>
    ...
    async getData() {
      ...
      switch (this.parentInfo) {
        case ParentInfoEnum.REPO:
          serviceCall = GitHubService.getTopStarredRepositoriesByPage(this.currentPage, this.perPage)
          break
        case ParentInfoEnum.ORG:
          serviceCall = GitHubService.getTopStarredOrganizationsByPage(this.currentPage, this.perPage)
          break
        case ParentInfoEnum.USER:
          serviceCall = GitHubService.getTopStarredUsersByPage(this.currentPage, this.perPage)
          break
        default:
          this.errored = true
          this.loading = false
        ...
        .then(response => {
          ...
          this.items = items.map(item => {
            if (this.parentInfo === ParentInfoEnum.REPO) {
              return { id: item.id, name: item.full_name, avatar_url: item.owner.avatar_url, star: item.stargazers_count }
            }
            return { id: item.id, name: item.login, avatar_url: item.avatar_url, star: item.followers || 0 }
          })
