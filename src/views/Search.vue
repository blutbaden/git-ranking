<template>
  <div :class="{'h-96' : loading}" class="relative container grid grid-cols-4 gap-1 mt-5 mx-auto">
    <div v-if="errored"
         class="backdrop-blur-sm w-full h-full flex items-center text-red-600 text-lg justify-center z-10 absolute">
    ...
    <template v-else>
      <div class="row-span-1 pr-8">
        ...
        <div class="mt-3">
          <router-link class="text-3xl text-blue-400 hover:text-blue-500" :to="`/${user.login}`">
            {{ user.login }}
          </router-link>
      ...
      <div class="col-span-3">
        ...
        <template v-else>
          ...
          <div v-for="item in items" :key="item.id" class="border border-gray-300 border-b-0">
            <router-link :to="`/${item.nameWithOwner}`">
              <div class="p-2 hover:bg-gray-50 cursor-pointer justify-between inline-flex w-full">
          ...
          <div class="border-b border-gray-300"></div>
          <a v-if="hasNext && !loadingMore"
             class="text-blue-400 text-sm py-5 flex items-center justify-center cursor-pointer hover:text-blue-500"
             @click="loadNext()">Load More</a>
    ...
    getRepoDetails(name) {
      GitHubService.getRepositoryDetails(name)
        .then(response => {
          ...
          this.repo = { id, homepage, description, created_at, forks, full_name, stargazers_count, html_url: response.html_url }
        })
        .catch(error => {
          console.error(error)
          this.errored = true
