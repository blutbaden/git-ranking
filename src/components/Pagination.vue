    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
      ...
      <div>
        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
          <a @click="setPage(1)" aria-current="page"
             :class="{'disabled bg-gray-50 text-gray-400': pager.currentPage === 1, 'cursor-pointer hover:bg-gray-50': pager.currentPage !== 1}"
             class="z-10 rounded-l-md relative inline-flex items-center px-4 py-2 border text-sm font-medium">
            First
          </a>
          <a @click="prevPage"
             :class="{'disabled bg-gray-50 text-gray-400': pager.currentPage === 1, 'cursor-pointer hover:bg-gray-50' : pager.currentPage > 1}"
          ...
          <div v-for="page in pager.pages" :key="page"
               :class="{'text-indigo-500 border-indigo-300 bg-indigo-50 disabled': pager.currentPage === page, 'cursor-pointer hover:bg-gray-50': pager.currentPage !== page}">
            <a class="z-10 relative inline-flex items-center px-4 py-2 border text-sm font-medium" @click="setPage(page)">{{page}}</a>
          </div>
          <a @click="nextPage"
             :class="{'disabled bg-gray-50 text-gray-400': pager.currentPage === totalPages, 'cursor-pointer hover:bg-gray-50' : pager.currentPage < totalPages}"
             class="relative inline-flex items-center px-2 py-2 border text-sm font-medium">
          ...
          <a @click="setPage(totalPages)" aria-current="page"
             :class="{'disabled bg-gray-50 text-gray-400': pager.currentPage === totalPages, 'cursor-pointer hover:bg-gray-50': pager.currentPage < totalPages}"
             class="z-10 rounded-r-md relative inline-flex items-center px-4 py-2 border text-sm font-medium">
            Last
          </a>
        </nav>
      </div>
